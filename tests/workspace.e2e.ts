...
const plannedSpend = (page: Page) =>
  page
    .locator(".plan-stats .stat")
    .filter({ hasText: "PLANNED SPEND" })
    .locator("strong");
...
async function openPlan(page: Page) {
  ...
  await expect(
    page.getByRole("heading", { name: "Your move, made manageable." }),
  ).toBeVisible();
}
...
async function addTask(page: Page, title: string, cost = 0) {
  ...
  await page
    .getByLabel("Estimated cost ($)", { exact: true })
    .fill(String(cost));
...
async function savedToken(page: Page) {
  const token = await page.evaluate(() =>
    localStorage.getItem("roomready-home"),
  );
  expect(token).toMatch(/^[0-9a-f-]{36}$/i);
  return token!;
}

test("scroll camera, task editing, budget and completion persist and sync between independent sessions", async ({
  page,
  browser,
}) => {
  const errors: string[] = [];
  ...
  await page.goto("/");
  await expect(
    page.getByRole("heading", { name: "A fresh start. Coming together." }),
  ).toBeVisible();
  const canvas = page.locator("canvas");
  await expect(canvas).toHaveAttribute("data-ready", "true");
  await page.screenshot({
    path: path.join(evidence, "desktop-home.png"),
    fullPage: false,
  });
  ...
  await page.screenshot({
    path: path.join(evidence, "desktop-scroll.png"),
    fullPage: false,
  });

  await openPlan(page);
  const baseline = Number(
    (await plannedSpend(page).innerText()).replace(/[^\d.-]/g, ""),
  );
  ...
  try {
    ...
    await page
      .getByRole("button", { name: `Edit ${originalTitle}`, exact: true })
      .click();
    ...
    await expect(
      taskRow(second, editedTitle).locator(".assignee"),
    ).toContainText("Casey");
    await expect(
      taskRow(second, editedTitle).getByRole("button", {
        name: `Edit ${editedTitle}`,
        exact: true,
      }),
    ).toContainText("$125");
    ...
    await expect(
      taskRow(page, editedTitle).getByRole("checkbox"),
    ).toBeChecked();
    await expect(
      taskRow(second, editedTitle).getByRole("checkbox"),
    ).toBeChecked();
    ...
    await expect(taskRow(page, editedTitle).locator(".assignee")).toContainText(
      "Casey",
    );
    await expect(plannedSpend(page)).toHaveText(dollars(baseline + 125));
    await page.screenshot({
      path: path.join(evidence, "desktop-plan.png"),
      fullPage: false,
    });
...
test("switching shared links in the same tab changes the active home and preserves isolation after reload", async ({
  page,
}) => {
  await page.goto("/");
  await openPlan(page);
  ...
  await page
    .getByRole("button", { name: "Home settings", exact: true })
    .click();
  await page
    .getByRole("button", { name: "Create a separate, empty home", exact: true })
    .click();
  await expect(
    page.getByRole("heading", { name: "A fresh start. Coming together." }),
  ).toBeVisible();
  ...
  await expect(
    taskRow(page, "E2E: added after switching back to A"),
  ).toHaveCount(0);
  await expect(plannedSpend(page)).toHaveText("$24");
  await page.reload();
  await openPlan(page);
...
test("an invalid shared link recovers a stable home instead of creating a new one on every reload", async ({
  page,
}) => {
  ...
  await expect(
    taskRow(page, "E2E: survives invalid-link recovery"),
  ).toBeVisible();
});

test("the focused room controls the next task and completing it updates the room plan", async ({
  page,
}) => {
  await page.goto("/");
  ...
  await expect(next.getByRole("heading")).toHaveText(
    "Clean appliances before moving in",
  );
  await next
    .getByRole("button", { name: "See your move plan", exact: true })
    .click();
  await expect(page.locator(".task-row")).toHaveCount(2);
  await expect(
    taskRow(page, "Label boxes by cabinet").getByRole("checkbox"),
  ).toBeChecked();
  await expect(
    taskRow(page, "Clean appliances before moving in").getByRole("checkbox"),
  ).not.toBeChecked();
  await expect(taskRow(page, "Choose a sofa that fits the room")).toHaveCount(
    0,
  );
});

test("mobile plan fallback and draft-only email remain usable and honestly labeled", async ({
  page,
}) => {
  ...
  await expect(page.locator("canvas")).toHaveAttribute("data-ready", "true");
  await page.screenshot({
    path: path.join(evidence, "mobile-home.png"),
    fullPage: false,
  });
  expect(
    await page.evaluate(
  ...
  await expect(
    page.getByRole("heading", { name: "Kitchen Make it yours." }),
  ).toBeVisible();
  ...
  await expect(
    page.getByRole("button", { name: "Review & send", exact: true }),
  ).toBeVisible();
  await page.reload();
  await page.getByRole("button", { name: "Inbox", exact: true }).click();
  await expect(page.getByText("DRAFT", { exact: true })).toBeVisible();
  await expect(page.getByText("SENT", { exact: true })).toHaveCount(0);
  await page.screenshot({
    path: path.join(evidence, "mobile-inbox.png"),
    fullPage: false,
  });
  ...
  await expect(page.getByText("Source checked", { exact: false })).toHaveCount(
    0,
  );
});

test("an unconfigured research provider returns its actual error and creates no successful result", async ({
  page,
}) => {
  await page.goto("/");
  await page.getByRole("button", { name: "Discover", exact: true }).click();
  ...
  const unavailable = page
    .locator(".integration-notice")
    .filter({ hasText: "Web research is awaiting a service connection." });
  test.skip(
    (await unavailable.count()) === 0,
    "Requires external research to be disabled; no paid call attempted.",
  );
  const sourceCount = await page.locator(".source-card").count();
  await page
    .getByRole("textbox", { name: "Research query or website" })
    .fill("Moving services in San Francisco");
  ...
  await expect(
    page.getByText("Research started. Results will appear here.", {
      exact: true,
    }),
  ).toHaveCount(0);
});

test("reduced motion keeps the scroll camera stable and unavailable WebGL offers selectable rooms", async ({
  browser,
}) => {
  ...
  try {
    ...
    await noGpu
      ...
      .click();
    await expect(
      noGpu.getByRole("heading", { name: "Bedroom Make it yours." }),
    ).toBeVisible();
  } finally {
...
test("guided scroll reveals a room task, preserves its plan context, and returns to the overview", async ({
  page,
}) => {
  await page.goto("/");
  await expect(page.locator("canvas")).toHaveAttribute("data-ready", "true");
  const scrollLength = await page
    .locator(".home-story")
    .evaluate((node) => (node as HTMLElement).offsetHeight - innerHeight);
  await page.mouse.wheel(0, scrollLength * 0.48);
  await expect(
    page.getByRole("heading", { name: "Kitchen Make it yours." }),
  ).toBeVisible();
  await expect(page.locator(".next-card h2")).toHaveText(
    "Label boxes by cabinet",
  );
  await page
    .locator(".next-card")
    .getByRole("button", { name: "See your move plan", exact: true })
    .click();
  await expect(page.locator(".task-row")).toHaveCount(2);
  await expect(taskRow(page, "Label boxes by cabinet")).toBeVisible();
  await page.getByRole("button", { name: "My home", exact: true }).click();
  await page
    .getByRole("button", { name: "View whole home", exact: false })
    .click();
  await expect(
    page.getByRole("heading", { name: "A fresh start. Coming together." }),
  ).toBeVisible();
...
test("workspace screens meet automated WCAG AA checks and settings support keyboard focus", async ({
  page,
}) => {
  await page.goto("/");
  await expect(page.locator("canvas")).toHaveAttribute("data-ready", "true");
  for (const screen of ["My home", "Move plan", "Discover", "Inbox"]) {
    await page.getByRole("button", { name: screen, exact: true }).click();
    const audit = await new AxeBuilder({ page })
      .withTags(["wcag2a", "wcag2aa", "wcag21aa"])
      .analyze();
    expect(audit.violations, `${screen} automated accessibility audit`).toEqual(
      [],
    );
  }
  ...
  for (let step = 0; step < 10; step++) {
    ...
    expect(
      // Native dialog traversal can visit browser chrome (reported as body).
      // It must never focus an interactive control in the inert background.
...
test("a 320px reflow keeps task creation and navigation visible without horizontal overflow", async ({
  page,
}) => {
  await page.setViewportSize({ width: 320, height: 640 });
  await page.goto("/");
  await openPlan(page);
  await addTask(page, "E2E: readable narrow-screen task", 17);
  const row = taskRow(page, "E2E: readable narrow-screen task");
  ...
  await page.screenshot({
    path: path.join(evidence, "narrow-plan.png"),
    fullPage: true,
  });
});
