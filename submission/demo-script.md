...
**Status:** the first app is public at [RoomReady](https://wonderful-vulture-63.convex.site), with its household workflow verified by browser tests. This is a rehearsal script, not a recorded demo. OpenAI, Firecrawl and AgentMail remain disabled, and no live provider result, video, social post or submission receipt is recorded. The sponsor segments below require activation and verification before using their success narration; the truthful fallback lines apply to the current deployment.

**Pitch:** “RoomReady turns a move into one shared home: every room, task, cost and supplier conversation in the same place.”

## Prepare the take

- Open [the public RoomReady app](https://wonderful-vulture-63.convex.site) in a fresh browser session. Use **Share home → Copy private home link** to open the same synthetic home in a second independent session. Keep that private capability out of the video.
...
- Before recording successful sponsor segments, activate the providers on backend `wonderful-vulture-63` and verify one real run per integration. None is currently verified. If using an earlier completed run to avoid waiting on camera, show its actual timestamp and identify it as a previous run.
...
| 0:35-1:00 | Open **Move plan → Add a task**, keep Kitchen selected and enter a small estimate. Open the task's cost/edit control, update cost and assignee, then **Save changes**. | “I can assign the task and include its estimated cost. The budget stays connected to the work instead of living in another spreadsheet.” | Persisted task; an arithmetic change the viewer can follow; estimated-cost label. |
...
| 1:45-2:10 | After provider verification, open the move assistant and select **Build my next steps**, or inspect an identified previous real run. Suggestions are added as unassigned tasks; inspect and edit one. | “OpenAI adds suggested next steps to the plan, using our home details and saved source evidence. They are marked for review, with no invented quoted costs.” | A real generated result, visible job state and review of the added task. There is no separate accept-suggestion step in this version. |
...
| Public repository | Available: [C-Icaro/roomready](https://github.com/C-Icaro/roomready), with [PR #1](https://github.com/C-Icaro/roomready/pull/1). Recheck access before submitting. |
...
| Live app | Available: [RoomReady](https://wonderful-vulture-63.convex.site), verified with seven browser scenarios against the first public deployment. Sponsor integrations remain inactive. |
| Demo video | Pending. Record and provide a publicly viewable link below three minutes; this script is not the video. |
...
## Current truthful short description

> RoomReady is a shared moving-home organizer with a scroll-driven 3D home. Households organize and assign room tasks, update estimated costs and see Convex live updates across sessions. Enquiry drafts are available. Firecrawl research, OpenAI planning and AgentMail sending/inbox paths are implemented but not yet activated or verified live.

Update this description only after real integration evidence exists. Do not claim awards, user adoption, savings or superiority to previous winners without evidence. The known links and ready-to-paste entry text are in [application.md](./application.md); the package is not yet submission-ready.
